...
echo ">>> Testing if publish to different file name works"
./publish \
--input test_file.txt \
...
echo ">>> Testing if publish in local dir with explicit file works"
./publish \
--input test_file.txt \
...
[[ ! -d adir ]] && echo "It seems no output is generated" && exit 1
[[ ! -f adir/yadir/test_file.txt ]] && echo "It seems no output is generated" && exit 1
