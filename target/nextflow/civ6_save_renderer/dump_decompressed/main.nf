...
thisConfig = processConfig(jsonSlurper.parseText('''{
    ...
    "resources" : [
      {
        ...
        "path" : "script.js",
        "is_executable" : true,
        "parent" : "file:/home/runner/work/civ6_pipeline/civ6_pipeline/src/civ6_save_renderer/dump_decompressed/"
      },
      {
        "type" : "file",
  ...
  "info" : {
    ...
    "platform" : "nextflow",
    "viash_version" : "0.7.3",
    "git_commit" : "a13a00a7144b3c5f75db80d8268bc7ad592d3ee0",
...
let par = {
  'input': $( if [ ! -z ${VIASH_PAR_INPUT+x} ]; then echo "String.raw\\`${VIASH_PAR_INPUT//\\`/\\`+\\"\\`\\"+String.raw\\`}\\`"; else echo undefined; fi ),
  'output': $( if [ ! -z ${VIASH_PAR_OUTPUT+x} ]; then echo "String.raw\\`${VIASH_PAR_OUTPUT//\\`/\\`+\\"\\`\\"+String.raw\\`}\\`"; else echo undefined; fi )
};
let meta = {
  'functionality_name': $( if [ ! -z ${VIASH_META_FUNCTIONALITY_NAME+x} ]; then echo "String.raw\\`${VIASH_META_FUNCTIONALITY_NAME//\\`/\\`+\\"\\`\\"+String.raw\\`}\\`"; else echo undefined; fi ),
  'resources_dir': $( if [ ! -z ${VIASH_META_RESOURCES_DIR+x} ]; then echo "String.raw\\`${VIASH_META_RESOURCES_DIR//\\`/\\`+\\"\\`\\"+String.raw\\`}\\`"; else echo undefined; fi ),
  'executable': $( if [ ! -z ${VIASH_META_EXECUTABLE+x} ]; then echo "String.raw\\`${VIASH_META_EXECUTABLE//\\`/\\`+\\"\\`\\"+String.raw\\`}\\`"; else echo undefined; fi ),
  'config': $( if [ ! -z ${VIASH_META_CONFIG+x} ]; then echo "String.raw\\`${VIASH_META_CONFIG//\\`/\\`+\\"\\`\\"+String.raw\\`}\\`"; else echo undefined; fi ),
  'temp_dir': $( if [ ! -z ${VIASH_META_TEMP_DIR+x} ]; then echo "String.raw\\`${VIASH_META_TEMP_DIR//\\`/\\`+\\"\\`\\"+String.raw\\`}\\`"; else echo undefined; fi ),
  'cpus': $( if [ ! -z ${VIASH_META_CPUS+x} ]; then echo "parseInt(String.raw\\`${VIASH_META_CPUS//\\`/\\`+\\"\\`\\"+String.raw\\`}\\`)"; else echo undefined; fi ),
  'memory_b': $( if [ ! -z ${VIASH_META_MEMORY_B+x} ]; then echo "parseInt(String.raw\\`${VIASH_META_MEMORY_B//\\`/\\`+\\"\\`\\"+String.raw\\`}\\`)"; else echo undefined; fi ),
  'memory_kb': $( if [ ! -z ${VIASH_META_MEMORY_KB+x} ]; then echo "parseInt(String.raw\\`${VIASH_META_MEMORY_KB//\\`/\\`+\\"\\`\\"+String.raw\\`}\\`)"; else echo undefined; fi ),
  'memory_mb': $( if [ ! -z ${VIASH_META_MEMORY_MB+x} ]; then echo "parseInt(String.raw\\`${VIASH_META_MEMORY_MB//\\`/\\`+\\"\\`\\"+String.raw\\`}\\`)"; else echo undefined; fi ),
  'memory_gb': $( if [ ! -z ${VIASH_META_MEMORY_GB+x} ]; then echo "parseInt(String.raw\\`${VIASH_META_MEMORY_GB//\\`/\\`+\\"\\`\\"+String.raw\\`}\\`)"; else echo undefined; fi ),
  'memory_tb': $( if [ ! -z ${VIASH_META_MEMORY_TB+x} ]; then echo "parseInt(String.raw\\`${VIASH_META_MEMORY_TB//\\`/\\`+\\"\\`\\"+String.raw\\`}\\`)"; else echo undefined; fi ),
  'memory_pb': $( if [ ! -z ${VIASH_META_MEMORY_PB+x} ]; then echo "parseInt(String.raw\\`${VIASH_META_MEMORY_PB//\\`/\\`+\\"\\`\\"+String.raw\\`}\\`)"; else echo undefined; fi )
};
...
thisDefaultProcessArgs = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "viash-io/civ6_pipeline/civ6_save_renderer/dump_decompressed",
    "tag" : "main_build"
  }
}'''),
  // auto settings
