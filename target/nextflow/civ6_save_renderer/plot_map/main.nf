...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component plot_map")
      println("exiting now...")
        exit 1
    }
  }
...
workflow plot_map {
  ...
  result_ = plot_map_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.plot_map
    .arguments
