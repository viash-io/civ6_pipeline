...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component combine_plots")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "combine_plots." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process combine_plots_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.combine_plots.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow combine_plots {
  ...
  result_ = combine_plots_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.combine_plots
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.combine_plots.output = "combine_plots.log"

  Channel.from(rootDir) \
    | filter { params.combine_plots.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.combine_plots.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | combine_plots
