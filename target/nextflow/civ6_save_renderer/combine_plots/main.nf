...
process combine_plots_process {
  ...
  stageInMode "symlink"
  container "${container}"
  publishDir "${params.output}/${id}/", mode: 'copy', overwrite: true, enabled: !params.test
  input:
...
workflow combine_plots {
    ...
    result_ = combine_plots_process(id_input_output_function_cli_) \
