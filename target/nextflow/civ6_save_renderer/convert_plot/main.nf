...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component convert_plot")
      println("exiting now...")
        exit 1
    }
  }
...
workflow {
  ...
  def inputFiles = params.convert_plot
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.convert_plot.output = "convert_plot.log"

  Channel.from(rootDir) \
    | filter { params.convert_plot.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.convert_plot.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | convert_plot
