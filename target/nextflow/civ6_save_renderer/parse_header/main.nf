...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component parse_header")
      println("exiting now...")
        exit 1
    }
  }
    ...
    .collect{ it ->
      ...
      def newValue =
        (it.value == "viash_no_value")
          ? "parse_header." + it.name + "." + extOrName
          : it.value
      def newName =
        (id != "")
          ? id + "." + newValue
...
process parse_header_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.parse_header.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow parse_header {
  ...
  result_ = parse_header_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.parse_header
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.parse_header.output = "parse_header.log"

  Channel.from(rootDir) \
    | filter { params.parse_header.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.parse_header.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | parse_header
