...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component parse_map")
      println("exiting now...")
        exit 1
    }
  }
...
process parse_map_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.parse_map.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow parse_map {
  ...
  result_ = parse_map_process(id_input_output_function_cli_params_)
...
workflow {
  ...
  def inputFiles = params.parse_map
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.parse_map.output = "parse_map.log"

  Channel.from(rootDir) \
    | filter { params.parse_map.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.parse_map.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | parse_map
